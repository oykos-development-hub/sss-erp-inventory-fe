...
const useGetReportInventoryList = () => {
  ...
  const fetchReportInventory = async (
    {organization_unit_id, office_id, date, is_lager}: paramsReportInventoryList,
    onSuccess: (date: any) => void,
  ) => {
    setLoading(true);
    try {
      const response = await fetch(GraphQL.getReportInventoryList, {organization_unit_id, office_id, date, is_lager});
